...
- При старте гоняются тесты, и в контейнере без TOKEN один из них падает - бот не стартует и уходит в цикл перезапусков. Так что `.env` с валидным токеном обязателен (issues #33, #45).
