...
Переменная `TOKEN` нужна: один тест проверяет, что токен задан (для прогона хватит любого значения, это заведено как issue #34). Сейчас всего 234 теста, прогон - пара секунд.

Бот и сам гоняет этот же набор перед стартом - если тесты красные, он не запустится.
...
| `test_main.py` | Запуск, on_ready |
| `test_integration.py` | Связки модулей вместе |

Тестовая база - временный файл, боевой `database.db` тесты не трогают. Изоляцию ещё можно усилить, это issue #67.
...
Известная болячка: `test_main.py` мутирует глобальный `main.bot` и частично проверяет копию кода вместо самого кода, issue #84. Если берётесь чинить тесты - начните с неё.
