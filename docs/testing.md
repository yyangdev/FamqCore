...
Переменная `TOKEN` нужна: один тест проверяет, что токен задан (для прогона хватит любого значения, это заведено как issue #34). Набор unittest выполняется за несколько секунд.
