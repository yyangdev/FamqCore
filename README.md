...
- Приём заявок двух типов (RP и CAPT) через кнопки и модальные формы
- Приватные тикет-каналы с кнопками: принять, отказать, вызвать на обзвон, закрыть
- Роль «Подал заявку» и уведомление в личные сообщения при подаче
- AFK-система: статус с причиной и временем возврата, автоответ на упоминания, статистика
- Команды статистики `!stats` и истории `!history` для администраторов
- Логи принятых и отклонённых заявок в отдельном канале
